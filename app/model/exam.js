...
module.exports = {
    ...
    searchExamTitle: function (examTitle, callback) {
        var query = "select * from online_test.exams JOIN online_test.question  ON question.examID = exams.examID   where examTitle like'%'" + examTitle + "'";
        con.query(query, callback);
    },
    searchExamDuration: function (examDuration, callback) {
        var query = "select * from online_test.exams where Duration like'%" + examDuration + "'";
        con.query(query, callback);
    },
    seacrhExamQuestion: function (questionBody, callback) {
        var query = "select * from online_test.question JOIN online_test.answer  ON question.qid = answer.qid where questionBody like'%" + questionBody + "%'";
        console.log(query)
        con.query(query, callback);
    },
